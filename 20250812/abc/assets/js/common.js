...
const transformTrand = (() => {
  ...
  const init = () => {
    const steps = container.querySelector('.trand__steps');
    const bgImg = document.querySelector('.content-section .trand__bg img');
    ScrollTrigger.create({
      trigger: container,
      start: 'top',
      ...
      onUpdate: (self) => {
        ...
        // Calculate translateY based on progress through thresholds (0% to -90%)
        const translateYPercent = currentIndex >= 0 ? (currentIndex / (progressThresholds.length - 1)) * 90 : 0;
        steps.style.transform = `translateY(-${translateYPercent}%)`;

        // Animate background image translateY to -30%
        if (bgImg) {
          const bgTranslateYPercent = currentIndex >= 0 ? (currentIndex / (progressThresholds.length - 1)) * 20 : 0;
          bgImg.style.transform = `translateY(-${bgTranslateYPercent}%)`;
        }
...
document.addEventListener('DOMContentLoaded', function () {
  const topButton = document.querySelector('.main-footer__top');
  if (topButton) {
    // Initially hide the button
    topButton.style.opacity = '0';
    topButton.style.visibility = 'hidden';
    topButton.style.transition = 'opacity 0.3s ease, visibility 0.3s ease';

    // Add click event listener
    topButton.addEventListener('click', goTop);

    // Add scroll event listener to show/hide button and handle footer positioning
    window.addEventListener('scroll', function () {
      ...
      if (scrollY > showThreshold) {
        ...
        if (footer) {
          ...
          // If footer is visible in viewport, position button at footer's top
          if (footerRect.top < windowHeight) {
            // Calculate distance from bottom of viewport to footer top
            const distanceFromBottom = windowHeight - footerRect.top;
            topButton.style.transform = `translateY(-${distanceFromBottom}px)`;
            topButton.classList.add('fixed');
          } else {
            // Reset position when footer is not visible
      ...
      '(min-width: 1025px)': function () {
        historyItems.forEach((item, i) => {
          ...
          historyMotion
            ...
            .to(image, 2, { css: { borderRadius: '16px' } })
            .to(image, 1.5, { x: 0 })
...
const subMainVisual = new Swiper('.submain-swiper__inner .swiper', {
  ...
  pagination: {
    ...
    renderBullet: function (index, className) {
      return `
      <div class="${className}" data-index="${index}">${['설비 부문', '기술 부문'][index]}</div>
      `;
    },
  },
  on: {
...
const subMainVisual2 = new Swiper('.submain-swiper__inner-2 .swiper', {
  spaceBetween: 30,
  effect: 'fade',
  centeredSlides: true,
  speed: 1000,
  ...
  pagination: {
    ...
    renderBullet: function (index, className) {
      return `
      <div class="${className}" data-index="${index}">${['3D 디지털트윈​', 'R&D 부문'][index]}</div>
      `;
    },
  },
  on: {
...
const swiper = new Swiper('.swiper--news-swiper', {
  ...
  navigation: {
    prevEl: '.pagenation-step__prev',
    nextEl: '.pagenation-step__next',
  },
...
document.addEventListener('DOMContentLoaded', function () {
  const limitText = (selector, max) => {
    ...
    const link = document.querySelectorAll('.progress-swiper__link');
    link.forEach((el) => {
  ...
  // 패밀리 사이트 토글 기능
  const familySiteLink = document.querySelector('.family-site__link');
  const familySite = document.querySelector('.family-site');

  if (familySiteLink && familySite) {
    familySiteLink.addEventListener('click', function (event) {
      event.preventDefault();
